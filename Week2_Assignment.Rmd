...
names(stepsTotal) <- "Total_steps" # change the name
...
data$ID <- rownames(data) # change rownames
meanAlldays$interval <- rownames(meanAlldays) # change rownames
missing <- data[is.na(data$steps),] # find the rows with missing values
fillMissing <- merge(meanAlldays, missing, by ="interval") # add the mean values per interval to each missing rows by merging
